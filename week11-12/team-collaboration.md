...
| 2026-05-14(목) | pair-a/pair-b 상호 구현 재현, page fault 검증 안정화와 stack growth 준비 | `pintos/userprog/process.c`, `pintos/userprog/exception.c`, `pintos/userprog/syscall.c`, `pintos/include/threads/thread.h`, `pintos/include/vm/anon.h`, `pintos/include/vm/vm.h`, `pintos/vm/vm.c`, `pintos/vm/anon.c` | - pair-a 13일 구현 재현: lazy load aux 구조체, `load_segment()`, `lazy_load_segment()`, `vm_try_handle_fault()`, `page_fault()`<br><br>- pair-b 13일 구현 재현: `anon_initializer()`, `vm_anon_init()`, `anon_swap_in()`, `anon_swap_out()`, `anon_destroy()`, `struct anon_page` swap 관련 필드, `struct page` 상태 필드<br><br>- 공통 구현/통합 검증: syscall 경로 user rsp 저장 필드, user buffer validation helper, `vm_stack_growth()` | 두 페어가 서로의 13일 구현을 재현하고 lifecycle 차이를 비교한다. lazy loading에 당장 필요한 anon 최소 기능, swap/eviction 전제 기능 중 후속 보류 항목, 14일 밤 `team` 병합 후보를 분리한 뒤 남는 시간에 invalid fault, write-protected fault, stack 후보 fault를 기록 |
