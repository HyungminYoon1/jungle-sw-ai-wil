...
| 세부 구현 일정 | `implementation-plan.md`의 날짜별 계획을 기준으로 진행 |
...
| 항목 | 우리 팀 규칙 |
|------|--------------|
| 브랜치 전략 | 개인 브랜치에서 작업 후 main(팀 통합/최종 제출 브랜치)로 PR하는 것을 원칙으로 함 |
| PR 리뷰 | 관련 테스트, 수정 파일/함수, 입력/출력 shape, 실패 처리, 실험 결과를 함께 설명 |
| PR 머지 기준 | 관련 테스트 통과 + 작성자가 구현 흐름 설명 가능 + 팀원 1명 이상 리뷰 |
